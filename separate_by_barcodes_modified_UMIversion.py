...
barcodeset = zip(fwdbarcode,revbarcode)
foutfiles = []
routfiles = []
tenntfiles = []
namesind = 0
...
for m in barcodeset:
	...
	#tennt = open(fnames[namesind]+ "_UMIS_"+m[0]+"_" + m[1] +".txt", 'w')
	foutfiles.append(ftempfile)
	routfiles.append(rtempfile)
	#tenntfiles.append(tennt)
	namesind+=1
			...
			if flag:
				...
				for pair in frb:
					...
					if (fb,rb) in barcodeset:
						if fastqtype:
							foutfiles[barcodeset.index((fb,rb))].write(othername.split("\t")[0] + "_" + rseq[len(rb):len(rb)+ul]  + "\n"+ fseq[len(fb):] + "\n" + othername.split("\t")[1] + "\n" + othername.split("\t")[2][len(fb):] + "\n" )
							routfiles[barcodeset.index((fb,rb))].write(name.split("\t")[0] + "_" + rseq[len(rb):len(rb)+ul] + "\n"+ rseq[ul+len(rb):] + "\n" + name.split("\t")[1]+"\n" + name.split("\t")[2][ul+len(rb):] + "\n" )
							#tenntfiles[barcodeset.index((fb,rb))].write(rseq[len(rb):len(rb)+20] + "\n")
						else:
